...
class URLClassifierSVM:
    ...
    def extract_enhanced_features(self, url):
        ...
        try:
            ...
            suspicious_keywords = ['login', 'signin', 'account', 'bank', 'secure', 'verify', 'update', 'confirm', 'password', 'credit', 'card']
    ...
    def predict(self, url):
        ...
        # Check for immediate red flags even on trusted domains
        has_suspicious_extension = numerical_features.get('has_suspicious_extension', 0)
        has_suspicious_keyword = numerical_features.get('has_suspicious_keyword', 0)
        high_entropy = numerical_features.get('entropy_score', 0) > 4.5
        many_special_chars = numerical_features.get('num_special_chars', 0) > 15
        
        # If URL has obvious malicious indicators, flag as malicious regardless of domain
        if has_suspicious_extension or (has_suspicious_keyword and (high_entropy or many_special_chars)):
            return 'malicious', 0.85, self._generate_reason(numerical_features, 'malicious', 0.85)
        
        # Check if known safe domain (only for truly benign URLs)
        if self.is_safe_domain(url):
            # Even safe domains need basic content analysis
            if not has_suspicious_keyword and not high_entropy and not many_special_chars:
                return 'safe', 0.95, 'Known safe domain with no suspicious patterns'
        
        # Use ML model for full analysis
