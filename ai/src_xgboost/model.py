...
class URLClassifier:
    ...
    def predict(self, url):
        ...
        has_suspicious_extension = features.get('has_suspicious_extension', False)
        has_suspicious_keyword = features.get('has_suspicious_keyword', False)
        many_special_chars = features.get('num_special_chars', 0) > 10
        many_digits = features.get('num_digits', 0) > 5
        
        # If URL has obvious malicious indicators, flag as malicious regardless of domain
        if has_suspicious_extension or (has_suspicious_keyword and (many_special_chars or many_digits)):
            return 'malicious', 0.85, self._generate_reason(features, 0.85)

        # Check if known safe domain (only for truly benign URLs)
        if self.is_safe_domain(url):
            if not has_suspicious_keyword and not many_special_chars and not many_digits:
